...
export default function MedicalIntelligence() {
  ...
  const handleExport = (format: string) => {
    ...
    // Use proper formatters based on the active tab
    let content;
    if (activeTab === 'chronology') {
      content = {
        title: "Medical Chronology Report",
        sections: formatChronologyForExport(result),
      ...
      content = {
        ...
        sections: formatBillsForExport(result),
      ...
      content = {
        ...
        sections: formatSummaryForExport(result),
  ...
  const formatChronologyForExport = (result: any) => {
    const sections = [];
    ...
    if (result.summary) {
      sections.push({ heading: "Summary", content: result.summary });
    }
    return sections.length > 0 ? sections : [{ content: "No chronology data available." }];
  };

  const formatBillsForExport = (result: any) => {
    const sections = [];
    if (result.bills && Array.isArray(result.bills)) {
      sections.push({
        ...
        content: result.bills.map((bill: any, idx: number) =>
          ...
          `   Billed: $${bill.billedAmount?.toFixed(2) || '0.00'} | Reasonable: $${bill.reasonableAmount?.toFixed(2) || '0.00'}`
        ).join('\n\n'),
      });
    }
    if (result.totalBilled !== undefined) {
      sections.push({
        heading: "Totals",
        content: `Total Billed: $${result.totalBilled?.toFixed(2) || '0.00'}\nTotal Reasonable: $${result.totalReasonable?.toFixed(2) || '0.00'}`,
      });
    }
    return sections.length > 0 ? sections : [{ content: "No bill data available." }];
  };

  const formatSummaryForExport = (result: any) => {
    const sections = [];
    if (result.chiefComplaint) {
      sections.push({ heading: "Chief Complaint", content: result.chiefComplaint });
    }
    if (result.currentStatus) {
      sections.push({ heading: "Current Status", content: result.currentStatus });
    }
    if (result.treatmentSummary) {
      sections.push({ heading: "Treatment Summary", content: result.treatmentSummary });
    }
    if (result.prognosis) {
      sections.push({ heading: "Prognosis", content: result.prognosis });
    }
    if (result.totalMedicalExpenses !== undefined) {
      sections.push({ heading: "Total Medical Expenses", content: `$${result.totalMedicalExpenses?.toFixed(2) || '0.00'}` });
    }
    return sections.length > 0 ? sections : [{ content: "No summary data available." }];
  };
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900">
        <div className="container mx-auto px-4 py-8">
          <div className="max-w-6xl mx-auto">
            ...
            <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as keyof MedicalResults)} className="space-y-6">
